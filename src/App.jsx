...
export default function App() {
  ...
  // Dynamic tab title
  useEffect(() => {
    const original = 'UJJWAL SINGHAL · AI-FIRST SDE 2 · VICE CITY';
    const away = '[ ◉ PAUSED ] Vice City misses you...';
    const back = '[ ◉ LIVE ] Welcome back, recruit.';
    let backTimer;
  ...
  const downloadCv = () => {
    ...
    link.download = personal.cvDownloadName;
    link.click();
  };
    ...
    <>
      ...
      <div className='fixed top-0 left-0 right-0 z-[900] h-[2px]' style={{ background: 'rgba(0,0,0,.3)' }}>
        <div
          ...
          style={{
            width: `${scroll}%`,
            background: 'linear-gradient(90deg,#FF6B00,#FF1F6D,#8B00FF,#00E5FF)',
            boxShadow: '0 0 8px #FF1F6D',
          }}
        />
      </div>
      ...
      {/* Cinematic letterbox */}
      <div
        className='fixed left-0 right-0 z-[800] bg-black transition-all duration-700'
      ...
      <div className='fixed z-[600] hidden md:flex flex-col gap-2 items-end' style={{ bottom: '140px', right: '24px' }}>
        ...
        <button
          ...
          style={{
            ...
            boxShadow: '0 0 14px rgba(57,255,20,.15)',
          }}
        >
          🤝 REFER ME
        </button>
      ...
      <div className={`transition-opacity duration-700 ${loaded ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <main>
          ...
          <About
            onView={() => {
              fire('about');
              triggerMission('about');
            }}
          />
          <Experience
            onView={() => {
              fire('experience');
              triggerMission('experience');
            }}
          />
          <Skills
            onView={() => {
              fire('skills');
              triggerMission('skills');
            }}
          />
          <Numbers
            onView={() => {
      ...
      {cheatMsg && (
        <div
          className='fixed top-16 left-1/2 z-[9000] -translate-x-1/2 pointer-events-none'
          style={{ animation: 'termSlideUp .2s ease' }}
        >
          <div
            className='font-mono text-[10px] md:text-[12px] tracking-[5px] px-6 py-3 text-center'
            style={{
          ...
          >
            <span className='text-white/40 mr-2'>CHEAT CODE:</span>
            {cheatMsg}
          </div>
        </div>
      )}
